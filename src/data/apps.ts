...
export const apps: App[] = [
  {
    ...
    description:
      "大学生活の中で感じていた「やることはあるのに、なかなか動けない」という悩みをきっかけに作ったiOS向けToDoアプリ。「今やること」を1つだけ前に出し、カードUIとタイマーによって迷わず着手しやすくすることを目指している。",
    problem:
      "普通のToDoアプリでは、タスクを整理できても自分はなかなか動けなかった。一覧で見えている状態そのものが心理的な負担になり、何から始めるか考えているうちに止まってしまうことが多かったため、やることを1つだけ見せ、制限時間を設けることで着手しやすくする設計にした。",
    challenge:
      "最前面には常に1つのタスクだけを表示するカードUIを採用し、まず行動できることを重視した。また、時間・分・秒で設定できるタイマー、テンプレート機能、今日できたことの記録、ウィジェット対応などを組み合わせ、始めやすさと続けやすさの両立を意識している。",
    techs: ["Swift", "SwiftUI"],
    status: "審査中",
